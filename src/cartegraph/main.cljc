(ns cartegraph.main
  "Kotodama WASM entrypoint for the Cartegraph clean-room actor (L4) — Clojure port.
  ...
  facts (`cartegraph.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cartegraph")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cartegraph.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cartegraph-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cartegraph-compat/ready)
